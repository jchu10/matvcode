...
% 
% [lookingTimes, valid, oofDiffs, qualitative, lookingAtStart] = ...
%    read_lookingtime_vcode(filename, lookawayThresh)
% [lookingTimes, valid, oofDiffs, qualitative, lookingAtStart, msArray, trialStartsOut] = ...
%    read_lookingtime_vcode(filename, lookawayThresh)
%
% INPUT:
...
% qualitative: struct with the following fields:
%   fussPeriods: cell array of times when child is fussing during this 
%     trial, relative to looking time. Letting start = start of trial or 
%     start of child's look to screen (if later) and end = start of
%     qualifying lookaway, 30s from start if no qualifying lookaway and
%     child is still looking, or end of trial if no qualifying lookaway, 
%     possible descriptions are: 
%        start (includes start but not end), 
%        entire (includes start and end), 
%        during (between start and end, contains neither), 
%        end (includes end but not start)
%   fussTimes: total time child spends fussing during this trial (ms)
%   talkPeriods: cell array of times parent is talking, categorized as for
%      fussPeriods
%   talkTimes: total time parent spends talking during trial (ms); NOTE: may
%      double-count any periods where 'talk' events overlap (in contrast
%      to fuss/peek)
%   peekPeriods: cell array of times parent is peeking, categorized as for
%      fussPeriods
%   peekTimes: total time parent spends peeking during trial (ms)
%   distractPositions: array of times of distractions that occurred
%     between start and end (as defined for fussPeriods), relative to 
%     end, in ms; e.g. 500 means 500ms before end of trial
%   talkTranscripts: cell array of transcripts (VCode note) for talk 
%     events that overlap this trial (start-end) at all.
% lookingAtStart: array of booleans, whether child is looking at the start 
%   of each trial
%
...
% frame.
%
% If there are over 5 output arguments specified, then the following are
% computed:
% 
