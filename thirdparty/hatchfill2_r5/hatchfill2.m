...
%     Iram Weinstein.
%
%   KS, 2/16: Workaround to allow padding of patch interiors so hatch doesn't
%   obscure boundaries
...
try
   ...
   commonprops = {'Color',color,'Clipping', 'off','Parent',A.Parent,'DisplayName',A.DisplayName,'Visible',vis};
...
if ~isempty(V) % if patch shown
   ...
   % Workaround to allow padding of patch interiors so hatch doesn't
   % obscure boundaries - KS 2/16
      pad = 1.5;
      if size(V,1) == 5 % outline of box
          V(:,1) = max(V(:,1), min(V(:,1)) + pad);
          V(:,1) = min(V(:,1), max(V(:,1)) - pad);
          V(:,2) = max(V(:,2), min(V(:,2)) + pad);
          V(:,2) = min(V(:,2), max(V(:,2)) - pad);
      else % blocks of 4 rows denoting corners
          V(1:4:end, 1) = V(1:4:end, 1) + pad;
          V(2:4:end, 1) = V(2:4:end, 1) + pad;
          V(3:4:end, 1) = V(3:4:end, 1) - pad;
          V(4:4:end, 1) = V(4:4:end, 1) - pad;

          V(1:4:end, 2) = V(1:4:end, 2) + pad;
          V(2:4:end, 2) = V(2:4:end, 2) - pad;
          V(3:4:end, 2) = V(3:4:end, 2) + pad;
          V(4:4:end, 2) = V(4:4:end, 2) - pad;
      end
